...
%token MAIN INT FLOAT CHAR DOUBLE VOID 
%token IF ELSE FOR WHILE RETURN PRINTLN 
%token ASSIGNOP BITOP 
%token NOT INCOP DECOP

%token<Symbol> ID CONST_INT CONST_FLOAT CONST_CHAR ADDOP MULOP RELOP LOGICOP
...
func_declaration : type_specifier ID LPAREN parameter_list RPAREN SEMICOLON
		{
			...
			$$ = new SymbolInfo($1-> getName() + " " +  $2-> getName() + "(" + $4-> getName() + ");", "nonterm");
...
compound_statement : LCURL statements RCURL
			{
				$$ = new SymbolInfo("{" + $2->getName() +  "}", "compound");
...
var_declaration : type_specifier declaration_list SEMICOLON
		{
			...
			$$ = new SymbolInfo(Type_found + " " + Var_found + ";", "nonterm");
